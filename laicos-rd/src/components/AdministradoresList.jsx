...
const AdminDashboard = () => {
  ...
  const [currentAdmin, setCurrentAdmin] = useState(null); // Estado para el administrador actual
  ...
  // Función para abrir el modal y setear el administrador actual (para editar o crear nuevo)
  ...
  const handleDelete = async (adminId) => {
    try {
      ...
      obtenerAdministradores(); // Actualiza la lista de administradores
    } catch (error) {
          ...
          {activeComponent === 'Administradores' && userRole === 'Administrador' && (
            <>
              <AdministradoresList
                ...
                handleDelete={handleDelete} // Pasar la función para eliminar
              />
              <Modal isOpen={modalOpen} onRequestClose={handleCloseModal}>
                <h2>{currentAdmin ? 'Editar Administrador' : 'Añadir Administrador'}</h2>
                {/* Aquí debes insertar el formulario para crear o editar administrador */}
